...
(function() {
  var client, color, crypt_hash, irc, random, settings;
  ...
  client.addListener("connect", function() {
    return console.log(color("STATUS: ", "bold"), color("Connected to " + settings.server, "green"));
  });
  ...
  client.addListener("join", function(channel, nick, msg) {
    if (nick === settings.nick) {
      ...
      return console.log(color("JOIN <" + channel + ">: " + nick, "blue+bold"));
    }
  });

  client.addListener("part", function(channel, nick, msg) {
    return console.log(color("PART <" + channel + ">: " + nick, "blue+bold"));
  });
  ...
  client.addListener("message" + settings.channel, function(nick, msg) {
    if (msg[0] === "@") {
      ...
      return console.log(color("<" + nick + ">", "yellow"), "" + msg);
    }
  });
  ...
  client.addListener("pm", function(from, msg) {
    if (from === settings.admin) {
      ...
      return console.log(color("UNAUTHORIZED PM:", "red+bold"), "<" + from + "> " + msg);
    }
  });
